...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    // 2) Verificate token
    const decoded = await promisify(jwt.verify)(token, process.env.JWT_SECRET, (req, res))
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    // validateBeforeSave will deactivate all the validators specified in the schema
    await user.save({ validateBeforeSave: false })

    // 3) Send it to user's email
    const resetURL = `${req.protocol}://${req.get('host')}/api/v1/users/resetPassword/${resetToken}`

    const message = `Forgot your password? Submit a PATCH request with your new password and passwordConfirm to: ${resetURL}.\nIf you dodn't forget your password, please ignore this email`
